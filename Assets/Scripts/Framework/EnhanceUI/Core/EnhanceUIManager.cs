...
namespace Framework.EnhanceUI.Core
{
    ...
    public class EnhanceUIManager : MonoBehaviour, IUILoader
    {
        ...
        [Header("资源加载配置")]
        [SerializeField] private string uiPrefabPath = "UI/Prefabs/";
        ...
        /// <param name="onComplete">完成回调</param>
        /// <param name="data">传递的数据</param>
        public void OpenUI(string uiName, UILoadOptions options, Action<EnhanceUIPanel> onComplete = null, object data = null)
        {
            if (options == null)
            {
                                              ...
                                              Action<EnhanceUIPanel> onComplete, UILoadOptions options = null)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        UIName = uiName,
                        Data = (object)data, // 这里会装箱，但只在异步模式下
                        LoadMode = loadMode,
                        Config = config,
                        Priority = options?.Priority ?? 0,
                        CanBeCancelled = options?.CanBeCancelled ?? true
                    };
        ...
        /// <param name="onComplete">完成回调</param>
        /// <returns>UI面板</returns>
        private EnhanceUIPanel ProcessSyncRequest<T>(string uiName, T data, UIConfigData config, Action<EnhanceUIPanel> onComplete)
        {
            try
            {
                // 同步加载UI
                                            ...
                                            Action<EnhanceUIPanel> onComplete, UILoadOptions options = null)
        {
            ...
            try
            {
                ...
                UILoadRequest request = new UILoadRequest(uiName, loadMode, data, config, 
                    options?.Priority ?? 0, options?.CanBeCancelled ?? true);
        ...
        /// <returns>是否成功关闭</returns>
        public bool CloseUI(string instanceId)
        {
                ...
                yield return StartCoroutine(LoadUIPrefabAsync(uiName, config.PrefabPath, 
                    (prefab) =>
                    {
                        if (prefab != null)
                        {
                            ...
                            {
                                DestroyImmediate(uiGO);
                                onFailure?.Invoke($"UI预制体缺少EnhanceUIPanel组件: {uiName}");
                            }
                        }
                        else
                        {
                            onFailure?.Invoke($"加载UI预制体失败: {uiName}");
                        }
                    }));
            }
            catch (Exception e)
            {
                onFailure?.Invoke($"异步加载UI异常: {uiName}, 错误: {e.Message}");
            }
        }
        ...
        private UIConfigData ApplyLoadOptions(UIConfigData config, UILoadOptions options)
        {
            ...
            UIConfigData newConfig = new UIConfigData
            {
                ...
                AnimationType = options.SkipAnimation ? UIAnimationType.None : config.AnimationType,
        ...
        public ManagerStatus GetManagerStatus()
        {
            return new ManagerStatus
            {
                ...
                LayerCount = layerManager.GetAllLayers().Count,
