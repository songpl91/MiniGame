...
{
    [AddComponentMenu("Layout/Grid Layout Group MiddleSort")]
    /// <summary>
    ...
    /// </remarks>
    public class GridLayoutGroupMiddleSort : LayoutGroup
    {
        /// <summary>
        ...
        public bool LastLineAlineCenter = false;
        ...
        protected GridLayoutGroupMiddleSort()
        { }
        ...
        private void SetCellsAlongAxis(int axis)
        {
            ...
            if (LastLineAlineCenter)
            {
                int lastNum = ((actualCellCountY) * (actualCellCountX)) - rectChildren.Count;
                if (startAxis == Axis.Horizontal)
                {
                    float offsetOrigion = startOffset.x + (cellSize[0] + spacing[0]) * (lastNum / 2);
                    float offsetX = offsetOrigion + (lastNum % 2 > 0 ? (cellSize[0] / 2) : 0);
                    ...
                    for (int i = 0; i < rectChildren.Count; i++)
                    {
                        ...
                        if (positionY == actualCellCountY - 1)
                        {

                            bool noMove1 = rectChildren.Count < constraintCount;//; actualCellCountY == 1;
                            bool noMove2 = rectChildren.Count % constraintCount == 0;
                            bool noMove3 = constraintCount % 2 != 0 && rectChildren.Count % constraintCount == 1;
                            bool noMove4 = constraintCount % 2 == 0 && rectChildren.Count % 2 == 0;

                            bool noMove = noMove1 || noMove2 || noMove3 || noMove4;


                            float offset = noMove ? 0 : spacing[0] / 2f;
                            SetChildAlongAxis(rectChildren[i], 0, offsetX + (cellSize[0] + spacing[0]) * positionX + offset, cellSize[0]);
                        }
                        else
                        {
                ...
                {
                    float offsetOrigion = startOffset.y + (cellSize[1] + spacing[1]) * (lastNum / 2);
                    float offsetY = offsetOrigion + (lastNum % 2 > 0 ? (cellSize[1] / 2) : 0);
                    for (int i = 0; i < rectChildren.Count; i++)
                    {
                        int positionX = i / cellsPerMainAxis;
                        int positionY = i % cellsPerMainAxis;
                        ...
                        if (positionX == actualCellCountX - 1)
                        {
                            SetChildAlongAxis(rectChildren[i], 1, offsetY + (cellSize[1] + spacing[1]) * positionY, cellSize[1]);
                        }
                        else
                        {
                            SetChildAlongAxis(rectChildren[i], 1, startOffset.y + (cellSize[1] + spacing[1]) * positionY, cellSize[1]);
