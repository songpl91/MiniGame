...
public class HyperlinkText : Text, IPointerClickHandler
{
    ...
    protected override void OnPopulateMesh(VertexHelper toFill)
    {
        ...
        foreach (var hrefInfo in m_HrefInfos)
        {
            ...
            for (int i = hrefInfo.startIndex, m = hrefInfo.endIndex; i < m; i++)
            {
                ...
                if (previousPos != Vector3.zero && (i - hrefInfo.startIndex) % 4 == 0 &&
                    pos.x < previousPos.x) // 换行重新添加包围框
                {
    ...
    string GetOutputText_Init(string outputText)
    {
        ...
        foreach (Match match in s_HrefRegex.Matches(outputText))
        {
            ...
            s_TextBuilder.Append("<i><b><color=#1790d5>"); // 超链接颜色
    ...
    string GetOutputText(string outputText, int currentVertCount)
    {
        ...
        foreach (Match match in s_HrefRegex.Matches(outputText))
        {
            ...
            {
                offset_Len =
                    (s_TextBuilder.Length - Regex.Replace(s_TextBuilder.ToString(), @"<(.*?)>", "").Length) * 4;
            }
            ...
            {
                startIndex = Regex.Replace(str, @"<(.*?)>", "").Length * 4 + vercCount_Offset_Start +
                             offset_Len, // 超链接里的文本起始顶点索引
